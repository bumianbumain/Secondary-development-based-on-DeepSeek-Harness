...
/**
 * Self-contained tsdown config for the enterprise sidebar-brand bundle.
 *
 * The package lives under "enterprise/" (outside "packages/client/"), so it does
 * NOT use the shared "clientBundle" preset. This config reproduces the two
 * artifacts the host's client-module system expects from any browser plugin:
 *   1. `lib/index.js` — the node half (Loader entry's apply, a no-op here).
 *   2. `lib/client.js` — the browser half, wrapped in the
 *      `window.__ModuleLoader__.load({ id, factory })` handoff, with every
 *      `@deepseek-ai/*` / react import kept external so the runtime module
 *      table (served by the web app) answers them.
 */
const PKG = '@my-company/sidebar-brand'
